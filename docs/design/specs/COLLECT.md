...
COLLECT-04  Collection scans the unit folder and registers `ART-xxx` artifact nodes for all discovered files with task-aligned titles and unit metadata (Open Hospitality rule).
...
COLLECT-07  Screening verdicts, summary, and artifact edges with task context materialize into subject node frontmatter and activity history.
