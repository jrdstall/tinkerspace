...
QSTORM-05  Question-to-question relationships are supported across canonical edge relations: `decomposes`, `explores`, `broadens`, `narrows`, `reframes`, `presupposes`, `challenges`, `tests`, `depends_on`, and `sibling`.
...
QSTORM-09  Updating a question node mutates question title, form (`open` <-> `closed`), and importance while preserving question ID, author attribution, and event history.
QSTORM-10  Question relationship updates modify or remove directional edges between question nodes while preserving `questions` edges to the subject.
